...
* Work out the real status from the dates and amounts.
*
* We don't require someone to manually mark an
* installment as due or overdue.
  */
  function getDisplayStatus(schedule: Schedule) {
  const expected = Number(schedule.expected_amount || 0);
...
(schedule) =>
getDisplayStatus(schedule) === "upcoming"
).length;

const dueCount = schedules.filter(
(schedule) =>
getDisplayStatus(schedule) === "due"
).length;

const overdueCount = schedules.filter(
(schedule) =>
getDisplayStatus(schedule) === "overdue"
).length;

const paidCount = schedules.filter(
(schedule) =>
getDisplayStatus(schedule) === "paid"
).length;

const outstandingAmount = schedules.reduce(
(total, schedule) => {
...
if (loading) {
return ( <main className="min-h-screen bg-gray-100 p-8"> <div className="mx-auto max-w-7xl"> <h1 className="text-3xl font-bold text-gray-900">
Payment Schedule </h1>
...
return ( <main className="min-h-screen bg-gray-100 p-8"> <div className="mx-auto max-w-7xl space-y-8">
